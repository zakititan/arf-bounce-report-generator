...
- **Unsuspension confirmation** — after clicking Save, the content script watches for up to 8s for a definitive verdict: visible error elements → **failed**, success indicators or the Unblock button disappearing → **confirmed**, nothing → **unknown**. Each verdict is relayed (AD tab → service worker → report page) and aggregated per unsuspend run, so the report page shows an explicit toast: "Unsuspension verified in Abuse Desk ✓ (N/N)", a failure list, or an unverified warning. Runs with a legacy extension stay silent instead of nagging
